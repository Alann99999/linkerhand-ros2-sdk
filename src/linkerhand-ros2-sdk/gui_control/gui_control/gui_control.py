...
from PyQt5.QtWidgets import (
    ...
    QFrame, QSplitter, QMessageBox, QTextEdit, QFileDialog, QComboBox
)
...
class HandControlGUI(QWidget):
    ...
    def create_preset_actions_panel(self):
        ...
        # 保存当前动作帧按钮
        self.save_frame_btn = QPushButton("保存当前动作帧")
        self.save_frame_btn.setProperty("category", "action")
        self.save_frame_btn.clicked.connect(self.on_save_current_frame_clicked)
        actions_layout.addWidget(self.save_frame_btn)
        
        layout.addLayout(actions_layout)
        
        # 加载已保存的动作帧
        load_frame_group = QGroupBox("加载已保存的动作帧")
        load_frame_layout = QHBoxLayout(load_frame_group)
        
        load_frame_layout.addWidget(QLabel("选择动作帧:"))
        self.frame_combo = QComboBox()
        self.frame_combo.currentIndexChanged.connect(self.on_frame_selected)
        load_frame_layout.addWidget(self.frame_combo)
        
        self.refresh_frames_btn = QPushButton("刷新列表")
        self.refresh_frames_btn.clicked.connect(self.refresh_movement_frames)
        load_frame_layout.addWidget(self.refresh_frames_btn)
        
        self.load_frame_btn = QPushButton("加载并应用")
        self.load_frame_btn.setProperty("category", "action")
        self.load_frame_btn.clicked.connect(self.on_load_frame_clicked)
        load_frame_layout.addWidget(self.load_frame_btn)
        
        layout.addWidget(load_frame_group)
        
        # 初始化加载动作帧列表
        self.refresh_movement_frames()
...
发布频率: {self.ros_manager.hz} Hz"""
    ...
    def on_save_current_frame_clicked(self):
        """保存当前动作帧到 movement 文件夹"""
        try:
            movement_dir = self.get_movement_dir()
            if not movement_dir:
                QMessageBox.warning(self, "错误", "无法找到 movement 文件夹")
                self.status_updated.emit("error", "无法找到 movement 文件夹")
                return
            ...
            # 生成文件名并直接保存到 movement 文件夹
            file_name = f"{self.hand_joint}_frame_{time.strftime('%Y%m%d_%H%M%S')}.json"
            file_path = os.path.join(movement_dir, file_name)

            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False)

            self.status_updated.emit("info", f"动作帧已保存: {file_path}")
            # 刷新动作帧列表
            self.refresh_movement_frames()
            # 选中新保存的文件
            index = self.frame_combo.findText(file_name)
            if index >= 0:
                self.frame_combo.setCurrentIndex(index)
            
            QMessageBox.information(self, "保存成功", f"动作帧已自动保存到：\n{file_path}")
        except Exception as e:
            self.status_updated.emit("error", f"保存失败: {str(e)}")
            QMessageBox.warning(self, "保存失败", f"无法保存动作帧: {str(e)}")
            print(f"Save error: {str(e)}", flush=True)

    def get_movement_dir(self):
        """获取 movement 文件夹的绝对路径（支持多种运行环境）"""
        try:
            # 方案1：使用 __file__ 相对路径
            try:
                movement_dir = os.path.join(
                    os.path.dirname(__file__), '..', 'movement'
                )
                movement_dir = os.path.abspath(movement_dir)
            ...
            try:
                ...
                movement_dir = current_file.parent.parent / 'movement'
                if movement_dir.exists():
                    return str(movement_dir)
            except:
                pass
            
            # 方案3：从工作目录查找
            for base_dir in [os.getcwd(), os.path.expanduser('~')]:
                movement_dir = os.path.join(base_dir, 'src', 'linkerhand-ros2-sdk', 'gui_control', 'movement')
                if os.path.exists(movement_dir):
                    return movement_dir
            
            # 方案4：通过环境推导
            if 'linker_hand_ros2_sdk' in os.getcwd():
                base = os.getcwd().split('linker_hand_ros2_sdk')[0] + 'linker_hand_ros2_sdk'
                movement_dir = os.path.join(base, 'src', 'linkerhand-ros2-sdk', 'gui_control', 'movement')
                if os.path.exists(movement_dir):
                    return movement_dir
            
            return None
        except Exception as e:
            self.status_updated.emit("error", f"获取 movement 文件夹失败: {str(e)}")
            return None

    def get_movement_files(self):
        """获取 movement 文件夹中的所有 JSON 文件"""
        try:
            movement_dir = self.get_movement_dir()
            
            if not movement_dir or not os.path.exists(movement_dir):
                print(f"Movement directory not found. Tried: {movement_dir}", flush=True)
                return []
            ...
            self.status_updated.emit("error", f"读取 movement 文件夹失败: {str(e)}")
            print(f"Error reading movement files: {str(e)}", flush=True)
            return []

    def refresh_movement_frames(self):
        """刷新动作帧下拉框列表"""
        ...
        movement_dir = self.get_movement_dir()
        if movement_dir:
            print(f"Movement dir found: {movement_dir}", flush=True)
        else:
            print("Movement dir not found", flush=True)
        
        files = self.get_movement_files()
        print(f"Found movement files: {files}", flush=True)
        
        if files:
            self.frame_combo.addItems(files)
            self.status_updated.emit("info", f"找到 {len(files)} 个保存的动作帧")
        else:
            self.frame_combo.addItem("(无保存的动作帧)")
            movement_dir_info = movement_dir if movement_dir else "不存在"
            msg = f"movement 文件夹中没有动作帧文件 (路径: {movement_dir_info})"
            self.status_updated.emit("info", msg)
        
        self.frame_combo.blockSignals(False)

    def on_frame_selected(self):
        """选择帧时的处理（仅显示文件名）"""
        current_file = self.frame_combo.currentText()
        if current_file and current_file != "(无保存的动作帧)":
            self.status_updated.emit("info", f"已选择动作帧: {current_file}")

    def on_load_frame_clicked(self):
        """加载并应用选中的动作帧"""
        try:
            current_file = self.frame_combo.currentText()
            if not current_file or current_file == "(无保存的动作帧)":
                QMessageBox.warning(self, "错误", "请先选择一个有效的动作帧文件")
                return
            ...
            file_path = os.path.join(movement_dir, current_file)
            ...
            if len(positions) != len(self.sliders):
                QMessageBox.warning(
                    self, "动作不匹配",
                    f"动作帧关节数量({len(positions)})与当前关节数量({len(self.sliders)})不匹配"
                )
                return
            ...
            for i, (slider, pos) in enumerate(zip(self.sliders, positions)):
                ...
                self.on_slider_value_changed(i, int(pos))
            
            # 发布关节状态
            self.publish_joint_state()
            
            self.status_updated.emit("info", f"已加载并应用动作帧: {current_file}")
            QMessageBox.information(self, "加载成功", f"动作帧已加载: {current_file}")
            
        except json.JSONDecodeError:
            self.status_updated.emit("error", f"JSON 文件格式错误: {current_file}")
            QMessageBox.warning(self, "错误", f"无法解析 JSON 文件: {current_file}")
        except Exception as e:
            self.status_updated.emit("error", f"加载失败: {str(e)}")
            QMessageBox.warning(self, "加载失败", f"无法加载动作帧: {str(e)}")
